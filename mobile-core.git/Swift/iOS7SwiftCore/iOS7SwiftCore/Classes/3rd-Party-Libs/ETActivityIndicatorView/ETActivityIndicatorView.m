...
-(void)nextCircle{
    if (circleNumber<maxCircleNumber){
        ...
        Circle *circle = [[Circle alloc] initWithFrame:CGRectMake((self.frame.size.width-circleSize)/2 - 1, self.frame.size.height-circleSize -1, circleSize + 4 - circleNumber, circleSize + 4 - circleNumber)];
        circle.color = color;
...
//        CGPathRelease(circlePath);
        ...
        /*
         * fix loader animation not working
         */
        CAKeyframeAnimation *pathAnimation = [CAKeyframeAnimation animationWithKeyPath:@"position"];
        pathAnimation.duration = 1.5;
        pathAnimation.repeatCount = CGFLOAT_MAX;
        pathAnimation.timingFunction = [CAMediaTimingFunction functionWithControlPoints:0.15f :0.60f :0.85f :0.4f];;
        ...
        pathAnimation.path = [UIBezierPath bezierPathWithArcCenter:CGPointMake(self.bounds.size.width/2, self.bounds.size.height/2)
                                                            radius:MIN(self.bounds.size.width - circle.bounds.size.width, self.bounds.size.height - circle.bounds.size.height)/2
                                                        startAngle:M_PI_2
                                                          endAngle:M_PI_2 + 2 * M_PI
                                                         clockwise:YES].CGPath;
        
        [circle.layer addAnimation:pathAnimation forKey:@"kSkypeCurveAnimationKey"];
