...
{
    MultiPart *part = [[JsonPart alloc] initWithName:name jsonPart:model];
    if (part) {
...
{
    MultiPart *part = [[StringPart alloc] initWithName:name stringPart:string];
    if (part) {
        [self.multiparts addObject:part];
    }
}
...
{
    MultiPart *part = [[ArrayPart alloc] initWithName:name arrayPart:array];
    if (part) {
        [self.multiparts addObject:part];
    }
}
          ...
          failure:(void (^)(AFHTTPRequestOperation *operation, NSError *error))failure
{
    [manager GET:self.baseUrl parameters:self.parameters success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        HttpResult *result = [[HttpResult alloc] initWithAFHTTPRequestOperation:operation];
        success(result);
